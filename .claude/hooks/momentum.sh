...
#
# Synced across the A777ance repos from localDNS (the reference harness).
#
...
# there is real momentum, forces it to continue the next-actions queue instead
# of yielding. It is designed so it CANNOT run away:
#
...
# sustains momentum, it does not override that rule.
set -uo pipefail
cd "${CLAUDE_PROJECT_DIR:-.}" 2>/dev/null || exit 0

# --- per-repo config -------------------------------------------------------
# QUEUE: where this repo's "default next actions" live. Empty = fall back to the
# repo's working notes (README / CLAUDE.md).
