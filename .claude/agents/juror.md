...
Follow the sampling doctrine (lazy anchor → derive in the open → commit), from a
single juror's seat:
...
- **Derive in the open.** Reason where the reasoning is *load-bearing in the
  answer*: show the actual steps that determine the result, not a detached
  "thinking" preamble you could delete without changing the answer. Reason from
  scratch, in your own path — your value to the panel is being **decorrelated**
  from the other jurors, so don't reach for the single most obvious framing if you
  see a better one.
